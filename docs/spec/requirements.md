...
| HM-REQ-0151 | [00.5](00.5-foundations.md) | `inspired_by` syntax and self-cite ban | A formal H-NNNN's `inspired_by` field (optional) MUST satisfy: ≤ 3 entries; each entry has a valid `id: I-NNNN` and `weight ∈ (0, 1]`; declared weights sum to 1.0; no cited I-NNNN's `proposer.hotkey` equals the citing H-NNNN's primary author hotkey; locked at T-ACC. Defends F8.a / F8.c per ADR 0024 | normative |
| HM-REQ-0152 | [00.5](00.5-foundations.md) | Stake-to-post for I-NNNN | Every I-NNNN under `/informal/` MUST declare `stake_tao ≥ ideator_min_stake`; the bond is escrowed at PR open, returned on `claimed`, forfeited to treasury on `expired`. Defends F8.a brainstorm sybil-spam per ADR 0024 | normative |
| HM-REQ-0153 | [00.5](00.5-foundations.md) | Staleness threshold for `inspired_by` | Every entry in `inspired_by` MUST reference an I-NNNN whose `proposed_at_block` is ≥ `ideator_staleness_blocks` earlier than the citing H-NNNN's PR-open block and whose `status` is `accepted`. Defends F8.c per ADR 0024 | normative |
| HM-REQ-0154 | [00.5](00.5-foundations.md) | Treat-as-data for I-NNNN | Every I-NNNN MUST declare `treat_as_data: true` in front matter; agents and validators treat all free-text fields as data; `prompt-injection.yml` scanner glob covers `/informal/`. Defends F8.d per ADR 0024 (F4 extension) | normative |
| HM-REQ-0160 | [00.5](00.5-foundations.md) | Threshold-gated execution | A hypothesis whose `sponsorship.min_pool_tao > 0` MUST satisfy `sponsorship.sum ≥ min_pool_tao` before any miner submission against it is accepted by validators; submissions to a `pending-funding` hypothesis are rejected at the validator pipeline. Defends F7.4 / F7.5 per ADR 0025 | normative |
| HM-REQ-0161 | [00.5](00.5-foundations.md) | `funding_window_blocks` expiry and refund | A `pending-funding` hypothesis whose `funding_window_blocks` elapses without `sponsorship.sum` clearing `min_pool_tao` MUST transition to `expired-funding` (terminal); pool refunds each sponsor their `bounty_tao` exactly; the `id` MUST NOT be reused. Defends F7.4 stall-as-permanent-DoS per ADR 0025 | normative |
| HM-REQ-0162 | [00.5](00.5-foundations.md) | min_pool_tao formula or audited override | A `sponsorship` block declaring `min_pool_tao` MUST set the value at the profile-formula default (`2 × budget_wallclock_usd × seeds_required × usd_per_tao`, `usd_per_tao` pinned at PR-merge block) or an explicit override justified in the PR description and accepted by the maintainer. Fixed at T-PFUND; no post-acceptance amendment. Defends F7.5 per ADR 0025 | normative |
