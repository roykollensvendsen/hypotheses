...
| HM-REQ-0161 | [00.5](00.5-foundations.md) | Funding window expiry and refund | A `pending-funding` hypothesis whose `funding_window_blocks` elapses without `sponsorship.sum` clearing `min_pool_tao` MUST transition to `expired-funding` (terminal); pool refunds each sponsor their `bounty_tao` exactly; the `id` MUST NOT be reused. Defends F7.4 stall-as-permanent-DoS per ADR 0025 | normative |
