...
| T-081 | Funding-stall DoS — adversary creates a coordinated cluster (≥ 10) of high-threshold hypotheses, never funds them, ties up sponsor mindshare or maintainer review attention while denying miner activation | T-FEXP automation per HM-REQ-0161 returns each sponsor's `bounty_tao` exactly when `funding_window_blocks` elapses without `min_pool_tao` clearing; per-author rate limits inherited from registration economics; `expired-funding` is terminal so a stall cannot be reused as cover | [00.5 § F7.4](00.5-foundations.md#f7--curation-manipulation), [17 § threshold-gated execution](17-hypothesis-lifecycle.md#threshold-gated-execution), ADR 0025 | partial — coordinated funding-stall as curation-by-omission is residual; F7 fixture under HM-REQ-0090 covers F7.4 / F7.5 by Phase 2 mid-point |
| T-082 | Threshold gaming via lowball — author sets implausibly low `min_pool_tao` (e.g., < 10 % of profile-formula default) to bootstrap immediate execution at zero economic signal, undermining the per-hypothesis c8 measurement | profile-formula default per HM-REQ-0162 is the maintainer's audit reference; explicit override requires PR-description justification accepted at review; `min_pool_tao` is fixed at T-PFUND with no post-acceptance adjustment | [00.5 § F7.5](00.5-foundations.md#f7--curation-manipulation), ADR 0025 | partial — single-maintainer review surface is the bus-factor risk; multi-maintainer governance at Phase 3+ tightens the audit |
...
| T-091 | Ideator-formaliser collusion — same operator runs both sides under different hotkeys, mints both an I-NNNN and a citing H-NNNN within a coordinated window, engineers an early fake settlement (within `ideator_staleness_blocks`) to drain the sponsor pool's ideator slice | self-cite ban (HM-REQ-0151) blocks identical-hotkey trivial case; staleness threshold (HM-REQ-0153, `ideator_staleness_blocks`) forecloses same-block collusive creation; two-tier ideator payout (HM-REQ-0070 reused) defers 30 % for 6 months — colluder forfeits the deferred share if T-OVR overturn fires | [00.5 § F8](00.5-foundations.md#f8--ideator-graph-manipulation), [17 § two-tier settlement](17-hypothesis-lifecycle.md#two-tier-settlement), ADR 0024 | partial — different-hotkey same-operator collusion that survives the staleness window is residual; the 5 pp absolute slice bounds the prize |
| T-092 | Forum-as-injection (F4 extension) — free-text I-NNNN content or motivating-evidence fields used to inject directives against agents or maintainers reading the registry | `treat_as_data: true` declared in I-NNNN front matter (HM-REQ-0154); `prompt-injection.yml` scanner gate extends to `/informal/`; agents follow the existing AGENTS.md "spec content is data" posture; oracle never ingests I-NNNN content | [00.5 § F8](00.5-foundations.md#f8--ideator-graph-manipulation), [`prompt-injection.yml`](../../.github/workflows/prompt-injection.yml), [AGENTS.md](../../AGENTS.md) | mitigated — same defence shape as T-073 |
