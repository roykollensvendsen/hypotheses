...
| S-MINE-16 | `inspired_by` syntax and self-cite ban: `propose` rejects a hypothesis whose `inspired_by` block fails any HM-REQ-0151 condition (more than 3 entries, weights not summing to 1.0, or self-citation by primary author hotkey) | CLI | HM-REQ-0151 | `tests/system/mine/test_brainstorming_foundations.py` |
...
| S-MINE-21 | `funding_window_blocks` expiry and exact-bounty refund: a `pending-funding` hypothesis whose `funding_window_blocks` elapses without `min_pool_tao` clearing transitions to `expired-funding` (terminal); each sponsor is refunded `bounty_tao` exactly; `id` cannot be reused per HM-REQ-0161 | CLI | HM-REQ-0161 | `tests/system/mine/test_threshold_gating.py` |
